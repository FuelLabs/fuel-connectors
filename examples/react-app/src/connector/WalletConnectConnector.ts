...
export class WalletConnectConnector extends PredicateConnector {
  ...
  public async _request_sign_message(message: string): Promise<string> {
    const { ethProvider } = await this.getProviders();
    const currentAccount = await this.getAccountAddress();
    ...
    const unsub = this.web3Modal.subscribeEvents(async (event) => {
      switch (event.data.event) {
        ...
        case 'CONNECT_SUCCESS': {
          console.log('[CONNECT_SUCCESS]', event.data);
          const { addresses = [] } = getAccount(wagmiConfig);
          console.log('[CONNECT_SUCCESS] addresses', addresses);

          console.log('[CONNECT_SUCCESS] fuelProvider', this.fuelProvider);
          const code = await BakoProvider.setup({
            provider: this.fuelProvider.url,
            address: addresses[0],
            encoder: TypeUser.EVM,
          })
            .then((provider) => {
              console.log('[CONNECT_SUCCESS] BakoProvider setup', provider);
            })
            .catch((err) => {
              console.error('[CONNECT_SUCCESS] BakoProvider setup error', err);
              throw new Error('Failed to setup BakoProvider');
            });

          console.log('[CONNECT_SUCCESS] code', code);
          const { ethProvider } = await this.getProviders();

          const signature = (await ethProvider?.request({
            method: 'personal_sign',
            params: [code, addresses[0]],
          })) as string;
          ...
          // await BakoProvider.create(this.fuelProvider.url, {
          //   address: addresses[0],
          //   token: `${CONNECTOR_AUTH_PREFIX}${this.sessionId}`,
          //   //@ts-ignore
          //   rootWallet: window?.localStorage.getItem(
          //     `${CONNECTOR_PREFIX}:predicate`
          //   ),
          // });
        ...
        case 'MODAL_CLOSE':
        case 'CONNECT_ERROR': {
          console.log('[CONNECT_ERROR]', event.data);
          unsub();
          break;
        }
      }
  ...
  public async sendTransaction(
    ...
    const { predicate, request, account, transactionRequest } =
