export const SIGNATURE_PENDING_ERROR = 'Signature is pending';

export const REOWN_APPKIT_NAMESPACE = '@appkit/active_namespace';
export const REOWN_APPKIT_CONNECTION_STATUS = '@appkit/connection_status';

export const ETHEREUM_ICON =
  'data:image/svg+xml;utf8;base64,';
