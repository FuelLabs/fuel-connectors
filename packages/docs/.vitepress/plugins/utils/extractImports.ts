 ...
 */
export const combineImportStatements = (
  importStatements: Record<string, Set<string>>,
) => {
  return Object.keys(importStatements)
    .map(
      ...
      (source) => {
        ...
        return `import ${importType}{ ${importItems} } from '${source.replace(
          TYPE_IMPORT_PREFIX,
          '',
        )}';`;
      },
 ...
 * ```
 */
export const validateSnippetContent = (
  snippetContent: string[],
  filepath: string,
) => {
  const allImportStatements = snippetContent.filter((line) =>
    IMPORT_START_REGEXP.test(line),
  );
  const validImportStatements = snippetContent.filter((line) =>
    IMPORT_REGEXP.test(line),
  );
...
export const collectImportStatements = (
  ...
  const allImportedItems: Set<string> = new Set();
  let currentImport = '';
  ...
  lines.forEach((line) => {
    ...
    if (collecting && line.includes('from')) {
      ...
      if (matches && matches.length >= 1) {
        ...
        const source = isTypeImport
          ? `${TYPE_IMPORT_PREFIX}${importSource}`
          : importSource;
